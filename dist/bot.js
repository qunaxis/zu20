...
bot.command('/status', async ctx => {
    const settings = await _index2.default.getStatus();
...
bot.command(`/warn`, async ctx => {
    ...
    const warnData = await _index2.default.setWarn(warn);
...
bot.command(`/infected`, async ctx => {
    ...
    if (message[1] != undefined) {
        let updValue = await _index2.default.setParameter('infected', message[1]);
...
bot.command(`/timer`, async ctx => {
    ...
    let message = ctx.message.text.split(' ');
    if (message[1] != undefined) {
        let updValue = await _index2.default.setParameter('timer', message[1]);
